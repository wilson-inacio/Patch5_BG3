...
games_df <- games_df %>%
  mutate(
    ...
    post = ifelse(month >= floor_date(patch_date, "month"), 1, 0),
    month_relative = round(as.numeric(difftime(month, floor_date(patch_date, "month"), units = "weeks") / 4))  # approx months
  )
...
games_plot <- games_df %>% 
  ...
  geom_vline(
    ...
    color = "black",
    linetype = "dotdash") +
  labs(title = "Weekly Player Count (Interpolated)", 
...
did_model <- lm(avg_players ~ treatment * post, data = games_df)
summary(did_model)
```
...
Going forward, it would be interesting to redo this project which access to higher-frequency data, as the data collected only allowed for a monthly analysis, having a weekly analysis might yield different results.<br>
...
An extension of this project could also be examining engagement effects on a longer time period.